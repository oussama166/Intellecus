...
if (!idConsult($_SESSION['Id_Client'], $conn)) {
...
function afficheclient($conn)
{
        ...
        $req = $conn->prepare("select entreprise.Nom_Entreprise as Nom,Flag FROM `client` ,entreprise WHERE client.Id_entreprise= entreprise.Id_Entreprise and client.Id_raison_social=2");
        $req->execute();
